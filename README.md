...
OpenVPNをインストールするか、TAPデバイスのドライバを単独でインストールします。  
 　参考) https://www.choge-blog.com/programming/windows-tapdevice/  
ちゃんとインストールできた場合、ネットワーク接続の一覧画面で見るとこんな感じです。  
(赤い✕印は、TAPアダプタの先にアプリが接続されてないことを示しています)  
  ![一覧](./tap1.png)  
### インストールしたアダプタをリネームする
インストールしたそのままだとXM6 TypeGからの指定が面倒なのでわかりやすい名前にリネームします。
TAPアダプタをクリックして選択→上に表示される「この接続の名前を変更する」をクリック→わかりやすい名前に変更します。  
　![リネームの図](./tap2.png)
### XM6 TypeGの設定画面でNereidを有効化して、TAPアダプタ名を設定する  
こんな感じで赤丸の個所を適宜設定します。先ほどリネームしたTAPアダプタ名を指定します。  
...
　例) ifconfig en0 192.168.10.100 netmask 255.255.255.0 up  
　　→この例では以、LANのDHCPで使用していない 102.168.10.100 を x68k に割り当てています  
  　　(我が家のWifiルータにぶら下がっているLANが192.168.10.xxxで、DHCPのアドレス割り当て範囲が101-200)  
...
　　→この例では我が家のWifiルータ(198.168.10.1)がゲートウェイ兼DNSなのでそのIPアドレスを設定しています  
・これらの設定はうまく動いたらAUTOEXEC.BATに記述しておくといちいち設定せずに済むので楽です   
### Windows上のネットワークアダプタのブリッジ設定  
こんな感じで、コントロールキーを押しながらTAPアダプタとホストPCのLANアダプタを選択して右クリックで「ブリッジ接続」を選択します。  
　![ブリッジ設定例](./bridge.png)  
...
　![再チャレンジ](./bridge3.png)  
## 確認  
・Windows側からHuman68kに設定したIPアドレスへpingを打ってみる  
・Human68k側から外部の適当なサイトへpingを打ってみる  
